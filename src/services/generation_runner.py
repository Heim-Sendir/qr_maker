...
class GenerationRunner:
    ...
    def run(self, merchants, mode: str):
        ...
        desc_text = ('Генерация QR-кодов'
                     if mode == 'qr' else 'Генерация QR с шаблоном')
    ...
    def _print_summary(self, mode, report_count, errors):
        print("\n==============================")
        print(f"Сгенерировано QR "
              f"{'с шаблоном' if mode == 'template' else 'без шаблона'}: "
              f"{report_count}")
